package AutomationFrameworks.keywords;
...
import AutomationFrameworks.reports.ExtentManager;
...
public class GenericKeywords {
	...
	public void openBrowser() {
		System.out.println("prop.getProperty(\"gridRun\"):"+prop.getProperty("gridRun"));
	...
	public void openBrowser1(){
		...
		if(prop.getProperty("gridRun").equals("Y")) {
			...
			if(browser.equals("Mozilla")){
				...
				// invoke profile 
				System.setProperty("webdriver.gecko.driver", "E:\\Shiva\\Selenium3\\HybridRediff\\geckodriver.exe");
	...
	public void validateUrl(){
		test.log(Status.INFO,"getting url ");
		compareResults(driver.getCurrentUrl());
		
	}
	
	
	public void compareResults(String actualResult){
		test.log(Status.INFO, "compareResults");
			System.out.println("actualResult:"+actualResult);
			
		String expectedResult=data.get(dataKey);
		System.out.println("expectedResult:"+expectedResult);
		if(!actualResult.equals(expectedResult))
			reportFailure("Actual result incorrect "+actualResult );
	...
	public void select(){
		...
		Select select=new Select(getObject(objectKey));
		select.selectByVisibleText(data.get(dataKey));
	...
	public void TimeOut(){
		try {
			...
			Thread.sleep(Integer.valueOf(data.get(dataKey))*1000);
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void explicitWait(){
		...
		// visibility of Object
		wait.until(ExpectedConditions.visibilityOf(getObject(objectKey)));
